...
Tin=583.15; %K saturation temperature at inlet pressure
...
mu=mu0*mu1;
%conversion from Pa*s to lb/h.ft

muc=mu*2419.088;

%surface tension (Eotvos) 

Tref=373.15;    %reference temperature for surface tension

Tcrit=647;  %Critical temperature of water 

ginit=0.0588*0.0000685; %surface tension at Tref

gsurf=ginit*(Tcrit-Tin)/(Tcrit-Tref);

% Calculation of Ybp from Levy model (1967)

C=0.015;

gc=1/32.174049*3600^2;

Ybp=C*sqrt(gsurf*gc*Dh*3.281*Rhoinl)/muc;
...
C=3.7711e-8;    %Pa.s.W/m.K Constant

lam2=C/(mu0*mu1)*((Tr/rhor)^2)*;
display(lam2);
