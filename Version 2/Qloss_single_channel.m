...
Cpmod=[4.217 4.211 4.198 4.189 4.184 4.181 4.179 4.178 4.178 4.179 4.180 4.182 4.184 4.186 4.188 4.191 4.195 4.199 4.203 4.209 4.214 4.217];

%% System properties
% pressure tube thermal conductivity

kzircPT=(7.51+(0.362e-3*Tbulk)-(0.618e-7*Tbulk^2)+(0.718e-11*Tbulk^3))*10^-3;

% calandria tube thermal conductivity

kzircCT=(7.51+(0.362e-3*Tmod)-(0.618e-7*Tmod^2)+(0.718e-11*Tmod^3))*10^-3;
...
betamodsys=interp1(Tmodref,betamod,Tbulk);
...
fx=@(x) (x*a)+(x^(7/6)*b)+(x^(4/3)*c)+Tmod-Tbulk;

Tdiff=fzero(@(x) fx,5);

Ts=Tdiff+Tmod;

Qloss=(Tbulk-Ts)/Rtotal*1000;
